...
import GlyphLabel from "./RSDGlyphLabel"
...
describe("Glyph", () => {
  ...
  describe("correct color handling for variants and values", () => {
    it("should render black by default", () => {
      ...
      expect(getByTestId("glyph-icon-bg")).toHaveAttribute(
        "fill",
        colors.gray.main,
      )
    })
    ...
    it("should render green for positive balance", async () => {
      const { getByTestId } = render(
        <GlyphLabel variant="balance" value={1.5} />,
      )
      expect(getByTestId("glyph-icon-bg")).toHaveAttribute(
        "fill",
        colors.green.main,
    ...
    it("should render black for negative balance", async () => {
      const { getByTestId } = render(
        <GlyphLabel variant="balance" value={-0.5} />,
      )
      expect(getByTestId("glyph-icon-bg")).toHaveAttribute(
        "fill",
        colors.black.main,
    ...
    it("should render purple for credit variant", async () => {
      const { getByTestId } = render(
        <GlyphLabel variant="credit" value={1.5} />,
      )
      expect(getByTestId("glyph-icon-bg")).toHaveAttribute(
        "fill",
        colors.purple.main,
