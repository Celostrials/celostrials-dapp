...
import { MersenneTwister } from '../../functions/mersenne';
...
function generateIdenticon(diameter: number, seed: number): HTMLDivElement {
...
interface IdenticonProps extends BoxProps {
  account: string
  diameter?: number
}

export const Identicon = ({ account, diameter, ...rest }: IdenticonProps) => {
  ...
  useEffect(() => {
    if (account && ref.current) {
      ...
      ref.current.appendChild(
        generateIdenticon(diameter ?? 16, parseInt(account.slice(2, 10), 16))
      )
    }
  }, [account, diameter])
